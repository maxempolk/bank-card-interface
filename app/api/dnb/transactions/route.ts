...
import type { DNBApiRequest, DNBTransactionsResponse, DNBTransactionRaw } from '@/types/dnb'
  ...
  try {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        let transactions: DNBTransactionRaw[] = []
        ...
        return NextResponse.json<DNBTransactionsResponse>({ transactions })
      }

      const errorBody = await response.text()
      console.log(
        `[transactions] API response ${response.status} for account=${accountNumber}. Body: ${errorBody.substring(0, 500)}`
      )
    ...
    console.error('[transactions] Error requesting API:', error)
    return NextResponse.json(
